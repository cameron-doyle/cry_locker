    ...
    class CryLocker
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                if (args.Length >= 2)
				{
                    //TODO get executing cmd location
                    cmd = GetCmd(args[0]);
                    path = args[1].Trim();
                    ...
                    if (d.Exists)
                    {
                        ...
                        byte winCheck = Encoding.Default.GetBytes(@":")[0];
                        byte linuxCheck = Encoding.Default.GetBytes(@"/")[0];
                        if ((pathBytes.Length <= 1 && pathBytes[0] != linuxCheck) && (pathBytes[0] != linuxCheck && pathBytes[2] != winCheck))
                        {
                            path = $"{loc}/{path}";
                        }
                    }*/
